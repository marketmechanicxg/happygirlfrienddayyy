...
(function (window, document) {
  ...
  function renderWishes() {
    ...
    const data = (SETTINGS.wishes && SETTINGS.wishes.length)
      ? SETTINGS.wishes
      : [{ icon: 'heart', text: 'Happy Girlfriend Day!' }];
  ...
  function initMisc() {
    ...
    // Girlfriend Day surprise button
    const gdayBtn = document.getElementById('gdayBtn');
    const gdayNote = document.getElementById('gdayNote');
    const gdayNoteClose = document.getElementById('gdayNoteClose');
    if (gdayBtn && gdayNote) {
      const openNote = (evt) => {
        const isOpen = gdayNote.classList.toggle('is-open');
        gdayBtn.setAttribute('aria-expanded', String(isOpen));
        gdayNote.setAttribute('aria-hidden', String(!isOpen));
        if (isOpen && !OCEAN.config.prefersReducedMotion && OCEAN.effects?.burstPetals) {
          const rect = gdayBtn.getBoundingClientRect();
          OCEAN.effects.burstPetals(rect.left + rect.width / 2, rect.top + rect.height / 2, 18);
        }
      };
      gdayBtn.addEventListener('click', openNote);
      gdayNoteClose?.addEventListener('click', () => {
        gdayNote.classList.remove('is-open');
        gdayBtn.setAttribute('aria-expanded', 'false');
        gdayNote.setAttribute('aria-hidden', 'true');
      });
    }
